...
```bash
local-doc search <query> [--repo <name>] [--exclude-location <pattern>]...
```
...
| Repo filter (positional) | `search refund product` | Only search "product" repo (legacy) |
| Repo filter (flag) | `search refund --repo product` | Named flag, same as positional |
