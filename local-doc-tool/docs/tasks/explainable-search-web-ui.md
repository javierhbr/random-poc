...
Greenfield web layer only — **zero changes to Go `code/`** (LLD constraint).
...
- [ ] 1.1 `GET /api/repos` with tolerant parse (deps: 0.1, est: ~25m)
...
- [ ] 1.2 Render selectable picker entries (deps: 0.2, 1.1, est: ~25m)
...
- [ ] 1.3 Per-repo indicators: has-graph + spec_count (deps: 1.2, est: ~15m)
...
- [ ] 1.4 Disable submit while zero repos selected (deps: 1.2, est: ~10m)
...
- [ ] 1.5 Surface `/api/repos` failure explicitly (deps: 1.1, 1.2, est: ~15m)
...
- [ ] 2.1 `POST /api/query` spawns Claude, returns sessionId, captures Claude `session_id` (deps: 0.1, est: ~40m)
...
- [ ] 2.2 Reject empty repos with 400 (deps: 2.1, est: ~10m)
...
- [ ] 2.3 SSE stream: line-by-line NDJSON → normalized events (deps: 2.1, est: ~45m)
...
- [ ] 2.4 Scope-pinned prompt with per-command scope flags (deps: 2.1, est: ~25m)
...
- [ ] 2.5 Fail explicitly on unusable answer / unparseable stream (deps: 2.3, est: ~20m)
...
- [ ] 2.6 Explicit error when `claude` is missing (deps: 2.1, est: ~10m)
...
- [ ] 2.7 Derive sources/graph from parsed Bash tool events (deps: 2b.3, est: ~10m)
...
- [ ] 2.8 Pre-grant Bash permission narrowly (deps: 2.1, est: ~15m)
...
- [ ] 2.9 Enforce single active session (deps: 2.1, est: ~15m)
...
- [ ] 2b.1 Classify Bash command by `local-search` subcommand (deps: 2.3, est: ~25m)
...
- [ ] 2b.2 Strip progress lines + `JSON.parse` result stdout (deps: 2b.1, est: ~25m)
...
- [ ] 2b.3 Route parsed results to `sources`/`graph` events (deps: 2b.2, est: ~15m)
...
- [ ] 2b.4 Fail-soft on unparseable results (deps: 2b.2, est: ~15m)
...
- [ ] 2b.5 Non-local-search commands → activity only (deps: 2b.1, est: ~10m)
...
- [ ] 3.1 Render answer markdown (deps: 2.3, 0.2, est: ~20m)
...
- [ ] 3.2 Running state + disable re-submission (deps: 2.3, est: ~15m)
...
- [ ] 3.3 Indicate "no answer produced" (deps: 3.1, est: ~10m)
...
- [ ] 3.4 Incremental (partial) answer rendering (deps: 3.1, est: ~15m)
...
- [ ] 4.1 Render node-link graph in Cytoscape.js (deps: 2b.3, 0.2, est: ~30m)
...
- [ ] 4.2 Mark contributing-source nodes (deps: 4.1, 5.1, est: ~20m)
...
- [ ] 4.3 Node styling + honest edge labeling (deps: 4.1, est: ~20m)
...
- [ ] 4.4 Empty-graph message (deps: 4.1, est: ~10m)
...
- [ ] 5.1 List sources with ranking fields (deps: 2b.3, 0.2, est: ~20m)
...
- [ ] 5.2 Render searched scope from `json context` (deps: 2b.3, est: ~15m)
...
- [ ] 5.3 Show missing repos with reason + fix (deps: 5.2, est: ~15m)
...
- [ ] 5.4 Make selected-but-unreached repos visible (deps: 5.2, est: ~15m)
...
- [ ] 5.5 Startup smoke test pins `json context` shape, degrade on failure (deps: 0.1, est: ~20m)
...
- [ ] 6.1 Static retrieval-pipeline diagram (deps: 0.2, est: ~20m)
...
- [ ] 7.1 Append tool/command activity timeline (deps: 2.3, 0.2, est: ~25m)
...
- [ ] 7.2 Show assistant progress text (deps: 7.1, est: ~10m)
...
- [ ] 7.3 Keep feed visible and live (deps: 7.1, est: ~10m)
...
- [ ] 7.4 Reflect phase from `status` events (deps: 7.1, est: ~10m)
...
- [ ] 7.5 Preserve full ordered activity log (deps: 7.1, est: ~10m)
