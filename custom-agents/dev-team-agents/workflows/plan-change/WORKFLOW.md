---
id: plan-change
description: Create the OpenSpec change folder with proposal.md, tasks.md, and handoff.md. Routes work to the right agent. Use when a new issue or feature is ready to be broken down and assigned.
...
## Do not use when
- The change folder already exists and tasks are assigned — use [skill:handoff] to update state
- Requirements are unclear — use [skill:propose] first
...
If proposal is missing, invoke [skill:propose] first or draft it inline:
```markdown
# Change: <change-id>
...
- **Complex change** (new service, schema migration, cross-package impact): request `design.md` from [role:tech-lead]
...
|---|------|-------|--------|-----------|
| 1 | Write proposal | [role:po] | done | — |
| 2 | Design architecture | [role:tech-lead] | pending | task 1 |
| 3 | Implement API endpoints | [role:sr-fullstack] | pending | task 2 |
| 4 | Implement UI components | [role:sr-fullstack] | pending | task 2 |
| 5 | Implement Flutter screens | [role:mobile] | pending | task 2 |
| 6 | Test and verify | [role:qa] | pending | tasks 3-5 |
| 7 | Deploy to GCP | [role:devops] | pending | task 6 |
```
...
- **Blocked on:** nothing
- **Next step:** Route task 2 to [role:tech-lead] (or task 3 to [role:sr-fullstack] if no design needed)
...
|-----------|-------------|
| Requirements need refinement | [role:po] |
| Complex change needing design | [role:tech-lead] |
| Simple web/backend change | [role:sr-fullstack] |
| Flutter/mobile change | [role:mobile] |
| Bug fix with clear scope | [role:sr-fullstack] |
