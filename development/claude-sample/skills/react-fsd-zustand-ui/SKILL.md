...
### Which atomic layer (atom/molecule/organism)?
→ Use `atomic-design` skill for component layer decisions

### Foyer-specific Zustand patterns (cache, createCacheSlice, useStoreData)?
→ Use `state-management` skill for foyer store architecture and cache patterns
