---
name: test-verify
description: Verify a completed change against its acceptance criteria. Traces each criterion to a test or manual check, documents evidence, and produces QA signoff. Use after implementation is complete before deployment. Invoke with /test-verify.
---
...
- Report the bug using the task API:
```
openspec_task({
  action: "task_bug",
  changeId: "<change-id>",
...
After writing verification.md with "Signoff: YES":
```
openspec_change({ action: "transition", projectCode: "<code>", changeId: "<id>" })
```
This advances from `verification` → `deployment`. Prerequisite: `verification.md` must contain "Signoff: YES".

### Step 6: Give signoff
If all criteria pass:
- Set `handoff.md` verification status to `passed`
- Set next owner to @devops
