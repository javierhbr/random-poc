...
### Step 1.5: Run Technical Shaping Protocol
Before identifying design surfaces, run the shaping protocol (SOUL.md):

**Impact assessment per layer:**
...
**Explicit decisions:** for each significant choice, record Decision / Rationale / Tradeoff / Alternatives considered.

**Risks:** populate the risk table — a `design.md` with no risks section is incomplete.

**Module boundaries:** list what modules are touched, what new modules are created, and what the public API surface of each is.

Only after completing the shaping protocol, proceed to Step 2.
...
- **Testability via `agent-browser`**: list the stable selectors (preferably `data-testid` on key interactive nodes) the QA agent will target, the snapshot tree shape it should expect, and any state observable via `agent-browser eval`. See `.claude/rules/agent-browser-ui-testing.md`.
...
### Step 4b: Transition phase state
After design.md is complete:
```
openspec_change({ action: "transition", projectCode: "<code>", changeId: "<id>" })
```
This advances from `design` → `implementation`. Prerequisite: `design.md` AND `tasks.md` must have content.
...
- [ ] For UI changes: agent-browser selectors, expected snapshot shape, and observable state are listed in Component Architecture
