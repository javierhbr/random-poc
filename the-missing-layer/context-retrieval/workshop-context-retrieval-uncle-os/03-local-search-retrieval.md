...
**Duration:** 20 minutes
**Source:** `company-os-starter/docs/user-guide/tutorials/03-search-your-workspace.md`,
`how-to/keep-search-fresh.md`, `reference/configuration.md`,
verified against `local-search` **0.3.14**
...
flowchart LR
    ...
    GR --> OS[Company OS / Team OS<br/>WHO OWNS + WHAT GOVERNS]
    OS --> ANS[Grounded answer]
...
> This module covers only how Local Search plugs into a Company OS workflow.
...
Specs (18):
  ...
  [uncle-os · FTS] company-os-starter/docs/user-guide/explanation/github-mcp-and-automation.md
    GitHub MCP and automation  .md
  ...
```
...
Also note the result annotation `([authority/canonical])` — Company OS metadata
surfacing in search results. That is Module 5 arriving early: the search tool
can tell you a document is canonical because the OS layer said so.
...
`local-search tags` is worth pausing on: those are the tags `company-os graph
build` derived in Module 2, now serving as retrieval facets. The generation
step and the retrieval step meet here.
...
does **not** cover *disk vs. upstream* — that is Problem A, and Module 5.
