...
flowchart TB
    ...
    subgraph A["Problem A — SYNCHRONIZATION of local copies"]
        A1["Is my copy the pinned revision?<br/>Owner: your sync discipline — not covered here"]
    end
...
Cases 1, 3, 4, and 5 need a synchronization layer. §12 asks for one. This
workshop does not supply it — say that plainly rather than implying the tools
cover it.
...
sequenceDiagram
    ...
    participant G as git (local copy)
    participant LS as local-search
    participant Q as Query

    D->>G: what revision is this copy, how old?
    G-->>D: rev + date (compare against upstream)
    D->>LS: repo list
    LS-->>D: LAST SCAN, LAST UPDATE, COMMIT, GRAPH
    alt copy stale or never scanned
        D->>G: refresh the copy
...
```bash
git -C <copy> log -1 --format='%h %ci'   # what revision, how old
...
Prescribe the *check* instead: compare the copy's revision against upstream
before you trust it, and refresh when they differ.
...
- **Do not oversell.** This module's honest conclusion is that two of the three
  responsibility boxes are *not* solved by any tool in the workshop. Problem A
  needs a sync discipline you build and own. Problem B is a human discipline.
