...
flowchart TB
    ...
    Q -->|"Who owns it?<br/>What governs it?"| OS["read the doc's frontmatter<br/>owner / authority fields"]
...
| "Who owns this / what governs it?" | the document's own frontmatter (`owner:`, `authority:`) |
...
Make it a session-closing habit:

```bash
graphify update .          # after any code change
```
...
```mermaid
flowchart LR
    G["Graphify<br/>what the code IS"] -->|gap| D["The written record<br/>what it SHOULD be,<br/>who owns it,<br/>whether it still applies"]

    style G fill:#f0fff0,stroke:#2ea043
    style D fill:#e8f4ff,stroke:#0366d6,stroke-width:2px
```

That gap is why Modules 1–2 exist. The graph is derived from code, so only the
written record carries intent. Pair a Graphify answer with a Local Search answer
and you have both halves.
...
- **Have the boundary sentence memorized:** *"Graphify knows what the code is,
  not what it was supposed to be."* It is what sends people back to the written
  record — and back to Modules 1–2.
