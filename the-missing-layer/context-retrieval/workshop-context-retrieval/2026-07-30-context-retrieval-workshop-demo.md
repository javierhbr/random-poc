...
**Date:** 2026-07-30
**Duration:** 70 minutes (4 modules + wrap)
...
> plus the verified behaviour of `local-search` 0.3.14 and `graphify` 0.9.26.
...
`platform-example-repo` show `—`. **Ask the room: is that context valid?**
...
§12 therefore asks for a **separate sync layer** — something that pins a
revision, materializes the copy read-only, and records a hash per file so drift
is detectable. That layer is a prerequisite for everything after this module;
it is not part of the retrieval tooling.
...
```bash
ls docs/product/discovery/            # team-private discovery briefs
ls docs/change-records/active/        # live PRDs
ls docs/reality/components/           # current-state truth per component
```
...
Two rules make the levels stay connected, and both are **enforced, not
suggested**:

- A PRD may only be opened from a brief that is `status: validated`, and it
  copies that brief's Problem/Success sections forward.
- The done-gate refuses while any `- [ ]` checklist item is unchecked, or while
  the component's reality doc has an `updated:` date older than the PRD's
  `created:` date.
...
Code repos are NOT modeled by the docs pipeline. The only binding back to
...
> **Note — what "hybrid FTS5 + BM25" actually means**
>
> Three separate things get compressed into that one phrase, and the header
> line `[source=fts · rank=bm25]` is the tool naming two of them.
>
> - **FTS5** is *where the match comes from*. It is SQLite's built-in
>   full-text search extension (version 5) — an inverted index mapping each
>   stemmed token to the documents containing it, so a query is a lookup, not
>   a scan of every file. It ships **inside SQLite**, which is why there is no
>   daemon, no server, and no network: the index is one file on your disk.
>   That is `source=fts`.
> - **BM25** is *how the matches get ordered*. "Best Match 25" is the standard
>   relevance function FTS5 ranks with. It scores a document on three signals:
>   term frequency with **saturation** (the 10th occurrence of "retry" adds
>   far less than the 2nd, so keyword stuffing cannot win), **inverse document
>   frequency** (a term appearing in few documents is worth more — "settlement"
>   outranks "the"), and **length normalisation** (a short doc matching your
>   term beats a long doc matching it incidentally). That is `rank=bm25`.
> - **Hybrid** is the part BM25 alone does *not* do. The keyword score is
>   blended with structural signals before you see the list: a centrality
>   boost for documents that are well-connected in the code graph
>   (`repos=N with graphs` is the tool telling you whether it had that signal
>   at all), per-scope weights so a spec outranks a derived artifact, and —
>   only when you pass `--semantic` — an embedding score for documents that
>   are *about* your question without containing your words.
>
> **Why it matters in this workshop:** BM25 is lexical. It cannot find a
> document that never uses your vocabulary. That is the exact seam Module 4
> covers — Graphify answers *how the code is wired*, which no amount of
> keyword ranking recovers. The two are complementary by construction, not
> competing implementations of the same idea.
...
| "Who owns this / what governs it?" | **the governed docs layer** |
...
## Module 5 — The governed docs layer as retrieval substrate (15 min)

### 5.1 What each layer contributes to retrieval

| Layer | What it makes retrievable |
|---|---|
| `standards/` | the baseline controls applied to everything |
...
| `ontology/` | canonical IDs — the vocabulary everything else refers to |
| `knowledge/` | read-only doc slices synced from repos that are *not* governed workspaces |
...
The four operations your pipeline needs:

| Operation | What it does |
|---|---|
| `sync` | materialize slices + write `workspace.lock.yaml` |
| `graph build` | refresh `knowledge/CLAUDE.md` so agents can find the slice |
| `validate` | run the gates, including `[8/8]` hash integrity |
| `status` | report drift — **run this BEFORE every sync** |
...
`status` reports exactly three drifts: **pin drift**, **slice-set
...
```text
frontmatter IDs  ──(graph build)──►  tags: [...] block + wikilinks
```
...
The last capability worth showing is retrieval **filtered by role**: a daily
view that answers "what is mine, right now" for a developer versus a product
owner, backed by the same ID list, the merged skill view, and a
`governance explain <component>` lookup.

Same workspace, ranked for who is asking.
...
```text
1. Is my context fresh?        workspace status            (pin / slice-set drift)
...
4. Who owns it, what governs?  governance explain <component>
...
7. Leave it current.           graphify update .     (after code changes)
                               graph build           (after frontmatter changes)
```

**The closing line:** the governed layers *organize* knowledge; the Context
...
- *"Where do code repos fit in the governed docs layer?"* — They don't,
  deliberately. The
