This section introduces an important operational constraint: context must be available locally so that Local Search and agents can consult it consistently, even when the original sources are distributed across multiple repositories.
...
This surfaces an important architectural decision: GitHub MCP can function as an acquisition layer, but Local Search remains the local indexing and retrieval layer. This makes it possible to try out MCP without making the entire system depend on its availability.
